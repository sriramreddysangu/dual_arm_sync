#!/usr/bin/env python3
"""
step_32.py  --  B-Spline Trajectory Generator   [DUAL ARM]
=====================================================================
INPUT  : s31_ik.json
OUTPUT : s32_trajectories.json
...
def eval_spline(cp_segs, duration):
    ...
    # one vector-valued spline over all NDOF joints (was a per-joint Python loop)
    spl = BSpline(knots, cp_flat, DEG, axis=0, extrapolate=True)
    pos = np.asarray(spl(s))
    vel = np.asarray(spl.derivative(1)(s)) / duration
    acc = np.asarray(spl.derivative(2)(s)) / duration**2
...
def main():
    print('\n' + '='*66)
    print('  STEP 32  --  B-Spline Trajectory Generator [DUAL ARM]')
    print('='*66)
    if not os.path.exists('s31_ik.json'):
        print('  s31_ik.json not found -- run step_31 first'); sys.exit(1)
    ...
    with open('s32_trajectories.json', 'w') as fh: json.dump(out, fh, indent=2)
    kb = os.path.getsize('s32_trajectories.json') / 1024.
    print(f'\n  Total B-spline time : {out["total_bspline_time_ms"]:.0f} ms')
    print(f'  Saved               : s32_trajectories.json ({kb:.0f} KB)')
    print(f'  Next                : ros2 run dual_arm_sync step_33\n')
