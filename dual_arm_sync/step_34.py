...
def _write(out, arm_names, fname='s34_synchronized.json'):
...
def _passthrough(rdata, arm_names, label, collision_free, mind, reason, kur_ms=0.):
    ...
    _write(out, arm_names)
    print(f'  Saved: s34_synchronized.json ({label})')
...
def main():
    print('\n' + '='*66)
    print('  STEP 34  --  Temporal Resolver: Path-Velocity Coordination [DUAL ARM]')
    print('='*66)
    if not os.path.exists('s32_trajectories.json'):
        print('  s32_trajectories.json not found'); sys.exit(1)
    with open('s32_trajectories.json') as fh: rdata = json.load(fh)
    ...
    if input_safe:
        ...
        if kcoll != 0:
            ...
            print(f'  Next : ros2 run dual_arm_sync step_35\n'); return
        ...
        print(f'  Saved: s34_synchronized.json (time-optimal passthrough)')
        print(f'  Next : ros2 run dual_arm_sync step_35\n'); return
    ...
    if feasible_kur:
        ...
        if kcoll == 0:
            ...
            print(f'  Saved: s34_synchronized.json (Kuramoto SUCCESS)')
    ...
    # (4) Genuinely unsafe -- honest fail, with the best-lag diagnostic.
    print(f'  Could not produce a collision-free trajectory by timing alone '
          f'(input had {in_coll} colliding steps).')
    if kur_rep.get('best_lag_diag'):
        print(f'  {kur_rep["best_lag_diag"]}')
        print('  -> if the residual sits near arc 1.0, one arm is holding at a pose that')
        print('     overlaps the other (deep swap); that needs B-spline retraction (step_43-style),')
        print('     or a less-swept IK branch in step_31.')
    _passthrough(rdata, arm_names, 'FAIL', False, in_mind, 'unresolved', kur_ms)
    print('  step_35 pre-flight will refuse to execute.\n')
