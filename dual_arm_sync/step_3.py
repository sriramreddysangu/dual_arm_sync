#!/usr/bin/env python3
"""
step_3.py  —  Simultaneous Collision Check: Find Exactly Which Segment Collides
═══════════════════════════════════════════════════════════════════════════════
...
SAFETY_MAR  = 0.12   # collision threshold
WARNING_MAR = 0.20   # warning zone
...
def check_timestep(oi: np.ndarray, oj: np.ndarray) -> List[Dict]:
    """Returns list of collision/warning events at this timestep."""
    events = []
...
def scan_pair(name_i  : str,
              ...
              n_seg   : int) -> Dict:
    """
    Scan every simultaneous timestep.
    Map each collision to its exact spline segment index.
    """
    ...
    for k in range(N):
        ...
        for e in evs:
            ...
            if e['collision']:
                ...
                any_c = True
                if p > worst_pen:
                    worst_pen = p; worst_k = k; worst_li = li; worst_lj = lj
            if e['warning']:
                lp_nw[(li,lj)] += 1; any_w = True

        # Map timestep → segment
    ...
    for seg in range(n_seg):
        ...
        segment_summary.append({
            ...
            'n_coll_steps' : seg_n_coll[seg],
            'n_warn_steps' : seg_n_warn[seg],
            ...
            'status'       : ('COLLISION' if seg_n_coll[seg] > 0
                               else 'WARNING' if seg_n_warn[seg] > 0
                               else 'SAFE'),
        })
    ...
    if worst_k >= 0:
        ...
        worst_info = {
            'arm_i_link'   : f'{name_i}.{LINK_NAMES[worst_li]} (link_{worst_li})',
            'arm_j_link'   : f'{name_j}.{LINK_NAMES[worst_lj]} (link_{worst_lj})',
    ...
    # Colliding segments list  (what step_4 needs)
...
def print_report(report: Dict):
    bar = '═' * 68; thin = '─' * 68
    print(f'\n{bar}\n  STEP 3  —  COLLISION REPORT (simultaneous motion)\n{bar}')
    ...
    for pr in report['pair_reports']:
        ...
        print(f'      Steps checked    : {pr["n_steps_checked"]}')
        print(f'      Collision steps  : {pr["n_collision_steps"]} ({pr["collision_pct"]}%)')
        print(f'      Warning  steps   : {pr["n_warning_steps"]}')
        ...
        if pr['first_collision_frac'] is not None:
            print(f'      First collision  : '
                  ...
                  f'segment={pr["first_collision_seg"]}')

        wp = pr['worst_penetration']
        if wp:
            print(f'      Worst penetration: {wp["penetration_m"]*100:.2f}cm  '
                  f'@ t={wp["time_s"]:.3f}s  seg={wp["segment"]}')
            print(f'        {wp["arm_i_link"]}')
            print(f'        ↔ {wp["arm_j_link"]}')
        ...
        for ss in pr['segment_summary']:
            ...
            bar_str += sym
            labels.append(f'seg{ss["segment"]}:{ss["status"][0]}')
        print(f'\n      SEGMENTS [{n_seg}]:  {bar_str}  '
              f'(█=COLLISION  ▒=WARNING  ░=SAFE)')
        print(f'      Labels:  {" | ".join(labels)}')

        # Colliding segments detail
        for ss in pr['segment_summary']:
            if ss['collision']:
                print(f'      ❌ seg{ss["segment"]} '
                      f'arc=[{ss["arc_start"]:.2f},{ss["arc_end"]:.2f}]  '
                      f'coll_steps={ss["n_coll_steps"]}  '
                      f'max_pen={ss["max_pen_m"]*100:.2f}cm')

        # Link pair table
        lps = pr['link_pair_summary']
        if lps:
            print(f'\n      LINK-PAIR DETAIL:')
            hdr = f'      {"Arm-i link":<34} {"Arm-j link":<34} {"MinDist":>8} {"Clearance":>10} {"Coll":>5}'
            print(hdr); print('      ' + '─'*(len(hdr)-6))
            for lp in lps[:8]:
                print(f'      {lp["arm_i_link"]:<34} {lp["arm_j_link"]:<34} '
                      f'{lp["min_distance_m"]*100:>6.2f}cm '
                      f'{lp["clearance_m"]*100:>8.2f}cm '
                      f'{lp["n_collision_steps"]:>5}')

    any_c = any(p['n_collision_steps'] > 0 for p in report['pair_reports'])
    any_w = any(p['n_warning_steps']   > 0 for p in report['pair_reports'])
    print(f'\n{bar}')
    if any_c:
        print(f'  ❌  COLLISION → step_4 will resolve via Kuramoto + alternating IK')
    elif any_w:
        print(f'  ⚠   WARNING  → step_4 Kuramoto timing should resolve')
    else:
        print(f'  ✅  ALL CLEAR')
    print(f'{bar}\n')
...
def main():
    print('\n' + '=' * 68)
    print('  STEP 3  —  Simultaneous Collision Check')
    print('=' * 68)

    if not os.path.exists('trajectories.json'):
        print('\n  ❌  trajectories.json not found — run step_2 first'); sys.exit(1)
    ...
    for name in arm_names:
        ...
        arc_r = np.array(tdata[name]['trajectory'].get('arc_fracs',
                         np.linspace(0,1,len(pos_r))), dtype=float)
        if len(pos_r) == max_n:
    ...
    pair_reports = []
    for (ni, nj) in pairs:
        print(f'\n  Scanning {ni} ↔ {nj}  ({max_n} steps simultaneously) ...')
