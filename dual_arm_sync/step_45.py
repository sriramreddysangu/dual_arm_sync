#!/usr/bin/env python3
"""
step_45.py  --  ROS2 Executor for Lagrangian Pipeline   [DUAL ARM]
========================================================
INPUT  : s44_synchronized.json + s40_ik.json
OUTPUT : s45_execution.json
Dual-arm version (imports _robot2x).
...
if _ROS_OK:
    class ExecNode(Node):
        def __init__(self, arm_names):
            super().__init__('step_45_executor')
        ...
        def execute(self, arm_pos, duration):
            ...
            self.get_logger().info(f'step_45: {n_steps} steps @ {RATE_HZ:.0f} Hz '
...
def main(args=None):
    print('\n' + '='*66)
    print('  STEP 45  --  ROS2 Executor for Lagrangian Pipeline [DUAL ARM]')
    print('='*66)
    for fname, step in [('s44_synchronized.json', 'step_44'), ('s40_ik.json', 'step_40')]:
        if not os.path.exists(fname): print(f'  {fname} not found'); sys.exit(1)
    with open('s44_synchronized.json') as fh: sdata = json.load(fh)
    ...
    report = {'arm_names': arm_names, 'duration_s': duration, 'execution': exec_res,
              'verification': verif, 'pipeline': 'lagrangian (step_4X) [DUAL ARM]'}
    print(f'\n  {"="*66}')
    for name in arm_names:
        v = verif[name]
        print(f'  [{name}]  j_err={v.get("joint_error_deg","?")} deg  '
              f'ee_err={v.get("ee_error_mm","?")} mm  ee_ok={v.get("ee_ok","?")}')
    with open('s45_execution.json', 'w') as fh: json.dump(report, fh, indent=2)
    print(f'\n  Saved: s45_execution.json\n')
