#!/usr/bin/env python3
"""
step_15.py  —  Synchronized Execution + Final Report  (4-arm)
═══════════════════════════════════════════════════════════════════════════════
Input  : synchronized_trajectories.json
Output : execution_report.json

LOGIC  (identical to step_5, generalised to N=4 arms and 6 pairs)
─────
1. Pre-flight: verify synchronized trajectory is collision-free (all 6 pairs)
2. Execute all 4 arms at 100 Hz lock-step via ROS2
...
try:
    ...
    _ROS_OK = False
    print('[step_15] no ROS2 — dry-run mode')
...
def preflight(arm_names, data):
    ...
    for i in range(len(arm_names)):
        for j in range(i+1, len(arm_names)):
            ...
            nc = 0; dmin = float('inf')
            for k in range(K):
                d = pair_min_dist(pi[k], bi, pj[k], bj); dmin = min(dmin, d)
    ...
    return len(issues)==0, {'passed': len(issues)==0, 'issues': issues,
                             'warnings': warnings, 'pair_check': pair_results}
...
def arm_metrics(name, traj):
    ...
    path_len = float(np.sum(np.linalg.norm(np.diff(ee_path,axis=0),axis=1))) if len(ee_path)>1 else 0.
    if len(pos)>1:
        dt    = dur/max(len(pos)-1,1)
        vel   = np.gradient(pos, dt, axis=0); acc = np.gradient(vel, dt, axis=0)
        max_v = float(np.max(np.abs(vel))); max_a = float(np.max(np.abs(acc)))
    else:
        max_v = max_a = 0.
...
if _ROS_OK:
    class ExecutorNode(Node):
        def __init__(self, arm_names):
            super().__init__('step_15_executor')
...
def dry_run(arm_names, arm_pos, duration):
    ...
    print(f'\n  DRY-RUN: {n_steps} steps  ({duration:.2f}s  @{RATE_HZ:.0f}Hz)  '
          f'{len(arm_names)} arms lock-step')
    for k in range(n_steps):
        if k % milestone == 0:
            pct = int(100*k/n_steps)
            q0  = arm_pos[arm_names[0]][min(k,len(arm_pos[arm_names[0]])-1)]
            print(f'    {pct:3d}%  {arm_names[0]}: [{", ".join(f"{v:.3f}" for v in q0)}]')
    print(f'    100%  done')
    return {'success':True,'mode':'dry_run','steps_sent':n_steps,
            'wall_time_s':None,
            'final_joints':{n:arm_pos[n][-1].tolist() for n in arm_names},'error':None}
...
def print_report(report):
    bar = '═'*68; thin='─'*68
    print(f'\n{bar}\n  STEP 15  —  FINAL EXECUTION REPORT  (4 arms)\n{bar}')
    pf = report['preflight']
    ...
    if not pf['passed']:
        ...
        print('  ❌  RESIDUAL COLLISIONS → increase MAX_REFINE in step_14')
    else:
        print('  ✅  ALL 6 PAIRS SAFE — 4-arm execution complete')
    print(f'{bar}\n')
...
def main(args=None):
    print('\n' + '='*68)
    print('  STEP 15  —  4-Arm Synchronized Execution + Final Report')
    print('='*68)
